...
class DCMI_noSSL:
    def __init__(self, args, file_name, feature_extractor, task_size, device):
        ...
        self.transform = AugmentationSequential(
            Denormalize(mean=torch.tensor([0.485, 0.456, 0.406]),
                        std=torch.tensor([0.229, 0.224, 0.225])),
            RandomResizedCrop(size=(64, 64), scale=(0.2, 1.)),
    ...
    def beforeTrain(self, current_task):
        ...
        self.testfolder = self.data_manager.get_dataset(self.test_transform, index=class_set[:self.numclass], train=False)

        self.train_loader = torch.utils.data.DataLoader(self.trainfolder, batch_size=self.args.batch_size,
                                                        shuffle=True, drop_last=True, num_workers=8)
        self.test_loader = torch.utils.data.DataLoader(self.testfolder, batch_size=self.args.batch_size,
        ...
        for epoch in prog_bar:
            for step, (images, target) in enumerate(self.train_loader):
                ...
                if prob < 0.6:
                    ...
                    linear = F.softmax(torch.mm(F.normalize(r_out_old['feat']), F.normalize(self.old_proto).T) * GAMMA,
                                       dim=1)
                ...
                log_sim = F.log_softmax(torch.mm(F.normalize(out_old['feat']), F.normalize(self.old_proto).T) * GAMMA,
                                        dim=1)
            ...
            sch_opt_g.step()
            sch_opt_d.step()
            info = 'Train G => round {}/{} => loss_g {:.3f}, loss_d {:.3f}'.format(epoch + 1, len(prog_bar),
    ...
    def imgSave(self):
        ...
        for i in range(self.old_class, self.numclass):
            img = self.testfolder.get_label_image_class(i, 36)
    ...
    def compute_test_features(self):
        self.old_model.eval()
        features, lbs = [], []
        dummy_loader = DataLoader(dataset=self.testfolder, shuffle=False, batch_size=512)
        tqdm_batch = tqdm(
            total=len(dummy_loader), desc=f"[Compute test features]"
        )
        for batch, (images, target) in enumerate(dummy_loader):
            images, target = images.to(self.device), target.to(self.device)
...
class DCMI_labelaug:
    ...
    def imgSave(self):
        ...
        for i in range(self.old_class):
            ...
            # old_label = i*torch.ones(36, device=self.device)
            # vis_img = self.netG.sample(old_label.long())
            # vis_img = normalize(vis_img, reverse=True)
            # save_image_batch(vis_img, save_path + "/div_cls_{}.png".format(i))
...
class DCMI_SupCon:
    ...
    def _update_representation(self, imgs, target, opt):
        if self.old_model is None:
            out = self.model(torch.cat([imgs, self.transform(imgs)]))
            loss_cls = nn.CrossEntropyLoss()(out['logits'][0] / self.args.temp, torch.cat([target, target]))
            loss_con = self.model.SupCon(out['proj'][:target.size(0)],
                                         out['proj'][target.size(0):], target)
...
def normalize(tensor, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), reverse=False):
    if reverse:
