...
class tinyImageNet(object):
    def download_data(self):
        ...
        base_folder = './dataset'
        if os.path.isdir(base_folder + '/tiny-imagenet-200'):
            print('Files already downloaded and verified')
        else:
            download_and_extract_archive(url, base_folder)
        train_dir = os.path.join('./dataset', 'tiny-imagenet-200', 'train')
        test_dir = os.path.join('./dataset', 'tiny-imagenet-200', 'val')
...
class DummyDataset(Dataset):
    ...
    def get_label_image_class(self, label, batch_size):
        img = self.images[self.labels == label]
        img = np.random.permutation(img)[:batch_size]
        trf = transforms.Compose([transforms.ToTensor(),
                                  transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])
        img = [trf(pil_loader(tmp)) for tmp in img]
        return torch.stack(img)

    def get_random_image(self, batch_size, class_range, rp=None):
        if rp is None:
            rp = np.random.permutation(self.images.shape[0])
        mask = (class_range[0] < self.labels[rp]) & (self.labels[rp] < class_range[1])
        lbs = self.labels[rp][mask][:batch_size]
        img = self.images[rp][mask][:batch_size]

        trf = transforms.Compose([transforms.ToTensor(),
                                  transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])
        img = [trf(pil_loader(tmp)) for tmp in img]
        return torch.stack(img), torch.tensor(lbs)
